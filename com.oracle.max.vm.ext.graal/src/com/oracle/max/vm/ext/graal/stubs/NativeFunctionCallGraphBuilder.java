...
class NativeFunctionCallGraphBuilder extends AbstractGraphBuilder {
    ...
    public NativeFunctionCallGraphBuilder(ClassMethodActor nativeMethod) {
        ...
        List<LocalNode> nativeFunctionCallArgs = createLocals(0, ((ClassMethodActor) MaxResolvedJavaMethod.getRiResolvedMethod(NativeStubGraphBuilder.nativeFunctionCall)).descriptor(), true);
        ...
        while (nativeMethodArg.hasNext()) {
            ...
            if (arg.kind() == com.oracle.graal.api.meta.Kind.Object) {
                assert !isCFunction;
        ...
        final com.oracle.graal.api.meta.Kind resultKind = KindMap.toGraalKind(WordUtil.ciKind(sig.resultDescriptor().toKind(), false));
        ValueNode[] jniArgsArray = jniArgs.toArray(new ValueNode[jniArgs.size()]);
        NativeCallNode nativeCall = append(graph.add(new NativeCallNode(function, jniArgsArray,
                        resultKind, MaxResolvedJavaMethod.get(nativeMethod))));
    ...
    private InvokeNode handlize(ValueNode object) {
        assert object.kind() == com.oracle.graal.api.meta.Kind.Object;
