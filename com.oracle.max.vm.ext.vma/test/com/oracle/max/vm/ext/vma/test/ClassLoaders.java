 ...
 * {@link New0} are located in the same package the <code>New0</code>
 * will always ultimately be found by {@link ClassLoader# findSystemClass}, but
 * in that case the two instances of <code>New0</code> will be loaded by
 ...
 * to a different directory containing the <code>New0</code> classes. Then the custom
...
public class ClassLoaders {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (classpath == null) {
            System.out.println("must set -cp to location of New0.class");
            System.exit(1);
        }
        CLA cla = new CLA();
        CLB clb = new CLB();
        if (verbose) {
            System.out.println("loading New0 with CLA");
        }
        Class<?> claAclass = cla.loadClass("com.oracle.max.vm.ext.vma.test.New0");
        ...
        if (verbose) {
            System.out.println("loading New0 with CLB");
        }
        Class<?> clbAclass = clb.loadClass("com.oracle.max.vm.ext.vma.test.New0");
