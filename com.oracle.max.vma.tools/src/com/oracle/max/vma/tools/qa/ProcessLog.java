...
/**
 * Reads a log file in the format generated by {@link CompactTextVMAdviceHandlerLog}.
 *
...
public class ProcessLog {
    ...
    private ProcessLog(String dataFile, boolean verbose, int maxLines) throws IOException {
        this.dataFile = dataFile;
        this.verbose = verbose;
        this.maxLines = maxLines;
    }

    public static TraceRun processTrace(String dataFile, boolean verbose, int maxLines) throws IOException {
        ProcessLog pt = new ProcessLog(dataFile, verbose, maxLines);
    ...
    private TraceRun doProcessTrace() throws IOException {
        ...
        adviceRecordList = new ArrayList<AdviceRecord>((int) (file.length() / AVG_LINE_LENGTH));

        BufferedReader reader = checkTimeOrdered(file);
    ...
    private void processTraceRecord() throws TraceException {
        ...
        if (TextVMAdviceHandlerLog.hasTimeAndThread(key)) {
            getTimeAndThread();
        } else if (TextVMAdviceHandlerLog.hasTime(key)) {
            getTime();
        }

        if (TextVMAdviceHandlerLog.hasId(key)) {
        ...
        switch (key) {
            ...
            case RESET_TIME:
                throw new TraceException("unordered log is not supported - use ConvertLog -unbatch");
