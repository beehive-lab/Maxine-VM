        ...
        if (newValue == VoidValue.VOID) {
            ...
            if (vm().isBootImageRelocated()) {
                if (newValue == null || newValue.isZero()) {
                    ...
                    try {
                        _teleObject = vm().makeTeleObject(reference);
                    ...
                    _teleNativeThread = vm().threadContaining(address);
                    if (_teleNativeThread != null) {
                        ...
                        if (_valueMode == ValueMode.REFERENCE || _valueMode == ValueMode.LITERAL_REFERENCE) {
                            ...
                            _teleTargetMethod = vm().makeTeleTargetMethod(newValue.toWord().asAddress());
        ...
        switch (_valueKind) {
            ...
            case UNCHECKED_CALL_POINT: {
                ...
                action = new InspectorAction(inspection(), "View Code at address") {
                    @Override
                    public void procedure() {
                        inspection().focus().setCodeLocation(vm().createCodeLocation(address), true);
                    }
                };
                break;
            }
