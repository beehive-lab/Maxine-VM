 ...
 * Company, Ltd.
 */
/*VCSID=9ce3d171-5f29-4a90-af23-5463781a3328*/
...
public final class IconFactory {
    ...
    private static class CrossIcon extends BlankIcon {
        ...
        public void paintIcon(Component component, Graphics g, int x, int y) {
            ...
            g2.drawLine(_MARGIN, _MARGIN, getIconWidth() - _MARGIN, getIconHeight() - _MARGIN);
            g2.drawLine(_MARGIN, getIconHeight() - _MARGIN, getIconWidth() - _MARGIN, _MARGIN);
            g2.translate(-x, -y);
            g2.dispose();
        }
    }
    ...
    private static class PolygonIcon extends BlankIcon {
        protected final Polygon _polygon = new Polygon();
    ...
    private static class UpArrowIcon extends PolygonIcon {
        UpArrowIcon(int width, int height) {
            super(width, height);
            _polygon.addPoint(width / 2, _MARGIN);
            _polygon.addPoint(width * 7 / 8, height / 3);
            _polygon.addPoint(width * 5 / 8, height / 3);
            _polygon.addPoint(width * 5 / 8, height - _MARGIN);
            _polygon.addPoint(width * 3 / 8, height - _MARGIN);
            _polygon.addPoint(width * 3 / 8, height / 3);
            _polygon.addPoint(width * 1 / 8, height / 3);
        }
    }
    ...
    private static class DownArrowIcon extends PolygonIcon {
        DownArrowIcon(int width, int height) {
            super(width, height);
            _polygon.addPoint(width / 2, height - _MARGIN);
            _polygon.addPoint(width * 7 / 8, height * 2 / 3);
            _polygon.addPoint(width * 5 / 8, height * 2 / 3);
            _polygon.addPoint(width * 5 / 8, _MARGIN);
            _polygon.addPoint(width * 3 / 8, _MARGIN);
            _polygon.addPoint(width * 3 / 8, height * 2 / 3);
            _polygon.addPoint(width * 1 / 8, height * 2 / 3);
        }
    }
    ...
    private static class LeftArrowIcon extends PolygonIcon {
        LeftArrowIcon(int width, int height) {
            super(width, height);
            _polygon.addPoint(width - _MARGIN, _MARGIN);
            _polygon.addPoint(_MARGIN, height / 2);
            _polygon.addPoint(width - _MARGIN, height - _MARGIN);
        }
    }
    ...
    private static class RightArrowIcon extends PolygonIcon {
        RightArrowIcon(int width, int height) {
            super(width, height);
            _polygon.addPoint(_MARGIN, _MARGIN);
            _polygon.addPoint(width - _MARGIN, height / 2);
            _polygon.addPoint(_MARGIN, height - _MARGIN);
        }
    }
    ...
    private static class LeftEndIcon extends PolygonIcon {
        LeftEndIcon(int width, int height) {
            super(width, height);
            _polygon.addPoint(width - _MARGIN, _MARGIN);
            _polygon.addPoint(2 * _MARGIN, height / 2);
            _polygon.addPoint(2 * _MARGIN, height - _MARGIN);
            _polygon.addPoint(_MARGIN, height - _MARGIN);
            _polygon.addPoint(_MARGIN, _MARGIN);
            _polygon.addPoint(2 * _MARGIN, _MARGIN);
            _polygon.addPoint(2 * _MARGIN, height / 2);
            _polygon.addPoint(width - _MARGIN, height - _MARGIN);
        }
    }
    ...
    private static class RightEndIcon extends PolygonIcon {
        RightEndIcon(int width, int height) {
            super(width, height);
            _polygon.addPoint(_MARGIN, _MARGIN);
            _polygon.addPoint(width - (2 * _MARGIN), height / 2);
            _polygon.addPoint(width - (2 * _MARGIN), height - _MARGIN);
            _polygon.addPoint(width - _MARGIN, height - _MARGIN);
            _polygon.addPoint(width - _MARGIN, _MARGIN);
            _polygon.addPoint(width - (2 * _MARGIN), _MARGIN);
            _polygon.addPoint(width - (2 * _MARGIN), height / 2);
            _polygon.addPoint(_MARGIN, height - _MARGIN);
        }
    }
