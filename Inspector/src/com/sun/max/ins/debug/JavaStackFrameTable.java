 ...
 */
public class JavaStackFrameTable extends InspectorTable {

    private final JavaStackFrameViewPreferences viewPreferences;
    private final JavaStackFrameTableModel tableModel;
    private final JavaStackFrameTableColumnModel columnModel;
     ...
     * @param thread TODO
     */
    public JavaStackFrameTable(Inspection inspection, JavaStackFrame javaStackFrame, final  MaxThread thread, JavaStackFrameViewPreferences viewPreferences) {
        super(inspection);
        this.viewPreferences = viewPreferences;
        this.thread = thread;
        this.tableModel = new JavaStackFrameTableModel(inspection, javaStackFrame);
        this.columnModel = new JavaStackFrameTableColumnModel(viewPreferences);
    ...
    protected InspectorPopupMenu getPopupMenu(final int row, final int col, MouseEvent mouseEvent) {
        if (maxVM().watchpointsEnabled() && col == JavaStackFrameColumnKind.TAG.ordinal()) {
     ...
     */
    private final class JavaStackFrameTableColumnModel extends InspectorTableColumnModel<JavaStackFrameColumnKind> {

        JavaStackFrameTableColumnModel(JavaStackFrameViewPreferences viewPreferences) {
            super(JavaStackFrameColumnKind.VALUES.length(), viewPreferences);
            addColumn(JavaStackFrameColumnKind.TAG, new TagRenderer(inspection()), null);
            addColumn(JavaStackFrameColumnKind.NAME, new NameRenderer(inspection()), null);
            addColumn(JavaStackFrameColumnKind.ADDRESS, new AddressRenderer(inspection()), null);
            addColumn(JavaStackFrameColumnKind.OFFSET_SP, new OffsetSPRenderer(inspection()), null);
            addColumn(JavaStackFrameColumnKind.OFFSET_FP, new OffsetFPRenderer(inspection()), null);
            addColumn(JavaStackFrameColumnKind.VALUE, new ValueRenderer(inspection()), null);
            addColumn(JavaStackFrameColumnKind.REGION, new RegionRenderer(inspection()), null);
        }
    }
     ...
     *
     */
    private final class JavaStackFrameTableModel extends InspectorMemoryTableModel {

        private final JavaStackFrame javaStackFrame;
        private final int frameSize;
        private final Slots slots;
        private final MemoryRegion[] regions;

        public JavaStackFrameTableModel(Inspection inspection,  JavaStackFrame javaStackFrame) {
        ...
        public int getColumnCount() {
            return JavaStackFrameColumnKind.VALUES.length();
        }
    ...
    private final class ValueRenderer extends DefaultTableCellRenderer implements Prober{
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
            ...
            if (label == null) {
                label = new WordValueLabel(inspection, ValueMode.INTEGER_REGISTER, JavaStackFrameTable.this) {
                    @Override
