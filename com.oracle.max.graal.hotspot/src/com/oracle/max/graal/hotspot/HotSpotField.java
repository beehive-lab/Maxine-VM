    ...
    public CiConstant constantValue(CiConstant receiver) {
        if (receiver == null) {
            ...
            if (Modifier.isFinal(accessFlags())) {
                Field field;
                try {
                    field = holder().toJava().getDeclaredField(name());
                    field.setAccessible(true);
                    Object value = field.get(receiver.asObject());
                    return CiConstant.forBoxed(kind(false), value);
                } catch (Exception e) {
                    throw new GraalInternalError(e).addContext("field", this);
                }
            }
        }
        return null;
