...
public class ExternalCommand {
    ...
    public final MaxineTester.Logs logs;
    public final String[] command;
    public final String[] env;

    public ExternalCommand(File workingDir, File stdin, MaxineTester.Logs logs, String[] command, String[] env) {
    ...
    public Result exec(boolean append, int timeout) {
        ...
        try {
            ...
            if (logs.base != null) {
                sb.append(append ? " >>" : " > ").append(logs.get(MaxineTester.Logs.STDOUT).getAbsolutePath());
                sb.append(append ? " 2>> " : " 2> ").append(logs.get(MaxineTester.Logs.STDERR).getAbsolutePath());
            } else {
                sb.append(" > /dev/null");
                sb.append(" 2>&1");
            }
            ...
            if (logs.base != null) {
                final PrintStream ps = new PrintStream(new FileOutputStream(logs.get(MaxineTester.Logs.COMMAND)));
    ...
    public class Result {
        ...
        public String checkError(Result other, OutputComparison comparison) {
            ...
            if (comparison.stdout && !Files.compareFiles(logs.get(MaxineTester.Logs.STDOUT), other.command().logs.get(MaxineTester.Logs.STDOUT), comparison.stdoutIgnore)) {
                return "Standard out " + logs.get(MaxineTester.Logs.STDOUT) + " and " + other.command().logs.get(MaxineTester.Logs.STDOUT) + " do not match";
            }
            if (comparison.stderr && !Files.compareFiles(logs.get(MaxineTester.Logs.STDERR), other.command().logs.get(MaxineTester.Logs.STDERR), comparison.stderrIgnore)) {
                return "Standard error " + logs.get(MaxineTester.Logs.STDERR) + " and " + other.command().logs.get(MaxineTester.Logs.STDERR) + " do not match";
            }
            return null;
        }
