...
public class ARMV7T1XTest extends MaxTestCase {
    ...
    public void initialiseFrameForCompilation() {
        log("The frame initialisation in ARMV7T1XTest is a bodge just to allow testing");
        codeAttr = new CodeAttribute(null, new byte[50], (char) 10, // TODO: compute max stack
                        (char) 8, CodeAttribute.NO_EXCEPTION_HANDLER_TABLE, LineNumberTable.EMPTY, LocalVariableTable.EMPTY, null);
    ...
    // Each test should set the contents of this array appropriately, it enables the
    // instruction under test to select the specific bit values for
    // comparison i.e. testing upper or lower 16bits for movt,movw and for testing specific
    // bits in the status register etc. concerning whether a carry has been set
    ...
    public ARMV7T1XTest() {
        try {
            log("ARMV7T1XTest begin instantiation ................................");
            ...
            log("RuntimeCompiler.baselineCompilerOption.setValue");
            if (initialised == false) {
                vmConfigurator.create();
                initialised = true;
            }
            log("vmConfigurator.created");
    ...
    public void testIncStack() throws Exception {
        ...
        log("testIncStack: Passed");
    }

    public void testAdjustReg() throws Exception {
        log("testAdjustReg: Start");
        boolean success = true;
        int assemblerStatements;
        long[] registerValues = null;
        ARMV7MacroAssembler masm = theCompiler.getMacroAssemblerUNITTEST();
        ...
        for (int i = 0; i < 7; i++) {
            if (registerValues[i] != expectedValues[i]) {
                log("Register " + i + " " + registerValues[i] + " Expected " + expectedValues[i]);
                success = false;
            } else {
                System.out.println("Register " + i + " as expected");
            }
        }
        assert success == true;
        log("testAdjustReg: Passed");
    }
    ...
    public void testPokeInt() throws Exception {
        ...
        /*
         * DestinationRegister stores the least significant word.
         * DestinationRegister+1 stores the most significant word of the two-word long value
         */
        assemblerStatements = masm.codeBuffer.position() / 4;
        registerValues = generateAndTest(assemblerStatements, expectedValues, testvalues, bitmasks);
        for (i = 0; i < 6; i++) {
            if (registerValues[i] == expectedValues[i]) {
                log("Register " + i + " got the correct value");
            } else {
                success = false;
                log("Failed: Incorrect value " + Long.toString(registerValues[i], 16) + " " + Long.toString(expectedValues[i], 16));
            }
        }
        assert (success == true);
        log("testPokeInt: Passed");
    }

    public void testAssignLong() throws Exception {
        log("testAssignLong: Start");
        long[] registerValues = null;
        boolean success = true;
        long gotVal = 0;
        int i, assemblerStatements;
        ...
        for (i = 0; i < 10; i++) {
            if (i % 2 == 0) {
                ...
                if (gotVal == expectedValues[i - 1]) {
                    log("OK got Correct Value");
                } else {
                    success = false;
                    log("FAILED incorrect value " + Long.toString(gotVal, 16) + " " + Long.toString(expectedValues[i - 1], 16));
                }
            }
        }
        assert (success == true);
        log("testAssignLong: Passed");
    }

    public void testPeekLong() throws Exception {
        log("testPeekLong: Start");
        /*
         * The general strategy for this test is to assign a long, push it onto the stack and then
         * do a peekLong to check the correct read the value.
         */
        long[] registerValues = null;
        boolean success = true;
        long gotVal = 0;
        ...
        assemblerStatements = masm.codeBuffer.position() / 4;
        registerValues = generateAndTest(assemblerStatements, expectedValues, testvalues, bitmasks);
        log("Stack " + Long.toString(registerValues[13], 16) + " Scratch " + Long.toString(registerValues[12], 16));
        for (i = 0; i < 10; i++) {
        ...
        assert (success == true);
        log("testPeekLong: Passed");
    }

    public void testPokeLong() throws Exception {
        log("testPokeLong: Start");
        /*
         * The general strategy for this test is to assign a long, push it onto the stack and then
         * do a peekLong to check the correct read the value.
         */
        ...
        registerValues = generateAndTest(assemblerStatements, expectedValues, testvalues, bitmasks);
        for (i = 0; i < 10; i++) {
            log("Register Values " + i + " " + Long.toString(registerValues[i], 16));
            if (i % 2 == 0) {
                gotVal = 0;
                gotVal = 0xffffffffL & registerValues[i];
            } else {
        ...
        assert (success == true);
        log("testPokeLong: Passed");
    }

    public void testPeekInt() throws Exception {
        log("testPeekInt: Start");
        long[] registerValues = null;
        boolean success = true;
        int i, assemblerStatements;
        ARMV7MacroAssembler masm = theCompiler.getMacroAssemblerUNITTEST();
        ...
        for (i = 0; i < 6; i++) {
            if (registerValues[i] == expectedValues[i]) {
                log("OK Register value " + i + " got Correct Value");
            } else {
                success = false;
        ...
        assert (success == true);
        log("testPeekInt: Passed");
    }

    @SuppressWarnings("cast")
    public void testPokeDouble() throws Exception {
        log("testPokeDouble: Start");
        long[] registerValues = null;
        boolean success = true;
        long gotVal = 0;
        int i, assemblerStatements;
        ...
        for (i = 0; i < 5; i++) {
            log("Doubles " + Double.longBitsToDouble(expectedValues[i]) + " AS HEX " + Long.toString(expectedValues[i], 16));
            theCompiler.assignDoubleTest(ARMV7.allRegisters[16 + i], Double.longBitsToDouble(expectedValues[i]));
        }
        ...
        for (i = 0; i < 10; i += 2) {
            ...
            if (gotVal == expectedValues[i / 2]) {
                log("OK got correct Value " + Double.longBitsToDouble(expectedValues[i / 2]));
            } else {
                success = false;
                log("FAILED incorrect value " + Long.toString(gotVal, 16) + " " + Long.toString(expectedValues[i / 2], 16) + " EXPECTED " +
                                Double.longBitsToDouble(expectedValues[i / 2]) + " GOT " + Double.longBitsToDouble(gotVal));
            }
        }
        assert (success == true);
        log("testPokeDouble: Passed");
    }

    @SuppressWarnings("cast")
    public void testPeekFloat() throws Exception {
        log("testPeekFloat: Start");
        long[] registerValues = null;
        boolean success = true;
        int i, assemblerStatements;
        ARMV7MacroAssembler masm = theCompiler.getMacroAssemblerUNITTEST();
        ...
        for (i = 0; i < 6; i++) {
            masm.mov32BitConstant(ARMV7.cpuRegisters[i], (int) expectedValues[i]);
            log("As longs " + expectedValues[i] + " As floats " + Float.intBitsToFloat((int) expectedValues[i]));
        }
        masm.push(ARMV7Assembler.ConditionFlag.Always, 4 | 8); // this is to check/debug issues about wrong address loaded
        masm.push(ARMV7Assembler.ConditionFlag.Always, 1); // index 5
        masm.push(ARMV7Assembler.ConditionFlag.Always, 2); // index 4
        ...
        for (i = 0; i < 6; i++) {
            if (registerValues[i] == expectedValues[i]) {
                log("OK Register value " + i + " got Correct Value " + Float.intBitsToFloat((int) expectedValues[i]));
            } else {
                success = false;
                log("FAILED incorrect value " + Long.toString(registerValues[i], 16) + " " + Long.toString(expectedValues[i], 16) + " EXPECTED " +
                                Float.intBitsToFloat((int) expectedValues[i]) + " GOT " + Float.intBitsToFloat((int) registerValues[i]));
            }
        }
        assert (success == true);
        log("testPeekFloat: Passed");
    }

    @SuppressWarnings("cast")
    public void testPokeFloat() throws Exception {
        log("testPokeFloat: Start");
        long[] registerValues = null;
        boolean success = true;
        int i, assemblerStatements;
        ARMV7MacroAssembler masm = theCompiler.getMacroAssemblerUNITTEST();
        ...
        for (i = 0; i < 6; i++) {
            if (registerValues[i] == expectedValues[i]) {
                log("OK Register " + i + " got correct value " + Float.intBitsToFloat((int) expectedValues[i]));
            } else {
                success = false;
                log("FAILED incorrect value " + Long.toString(registerValues[i], 16) + " " + Long.toString(expectedValues[i], 16) + " EXPECTED " +
                                Float.intBitsToFloat((int) expectedValues[i]) + " GOT " + Float.intBitsToFloat((int) registerValues[i]));
    ...
    public void testAssignDouble() throws Exception {
        ...
        expectedValues[4] = (long) Double.doubleToRawLongBits(-100.75);
        for (i = 0; i < 5; i++) {
            log("As doubles " + Double.longBitsToDouble(expectedValues[i]) + " As hex " + Long.toString(expectedValues[i], 16));
            theCompiler.assignDoubleTest(ARMV7.allRegisters[16 + i], Double.longBitsToDouble(expectedValues[i]));
        }
        for (i = 0; i <= 9; i++) {
            masm.mov32BitConstant(ARMV7.cpuRegisters[i], -25);
    ...
    public void testPeekDouble() throws Exception {
        ...
        assert (success == true);
        log("testPeekDouble: Passed");
    }

    public void testdo_lconst() throws Exception {
        log("testdo_lconst: Start");
        long[] registerValues = null;
        long gotVal = 0;
        int assemblerStatements;
        ARMV7MacroAssembler masm = theCompiler.getMacroAssemblerUNITTEST();
        ...
        log(Long.toHexString(registerValues[0]) + " " + Long.toHexString(registerValues[1]));
        log(Long.toHexString(gotVal) + " EXPECTED  0xffffffff0000ffffL");
        log(Long.toHexString(registerValues[8]) + " EXPECTED " + Long.toHexString(expectedValues[8]));
        log(Long.toHexString(registerValues[9]) + " EXPECTED " + Long.toHexString(expectedValues[9]));
        log("STACK " + registerValues[2] + " " + registerValues[3]);
        ...
        log("testdo_lconst: Passed");
    }

    public void testdo_dconst() throws Exception {
        log("testdo_dconst: Start");
        long[] registerValues = null;
        long gotVal = 0;
        ...
        log(Long.toHexString(gotVal) + " EXPECTED  " + Long.toHexString(tmp));
        log(Long.toHexString(registerValues[8]) + " EXPECTED " + Long.toHexString(expectedValues[8]));
        log(Long.toHexString(registerValues[9]) + " EXPECTED " + Long.toHexString(expectedValues[9]));
        log("STACK " + registerValues[2] + " " + registerValues[3]);
        ...
        // assert(registerValues[9] == 1);
        assert (registerValues[2] - registerValues[3] == 8);
        log("testdo_dconst: Passed");
    }

    @SuppressWarnings("cast")
    public void testdo_fconst() throws Exception {
        log("testdo_fconst: Start");
        long[] registerValues = null;
        long gotVal = 0;
        ...
        log(Long.toHexString(registerValues[0]));
        log(Long.toHexString(gotVal) + " EXPECTED  " + Integer.toHexString(Float.floatToRawIntBits(myVal)));
        log("STACK " + registerValues[2] + " " + registerValues[3]);
        assert (gotVal == (long) Float.floatToRawIntBits(myVal));
        assert (registerValues[2] - registerValues[3] == 4);
        log("testdo_fconst: Passed");
    }

    public void testdo_load() throws Exception {
        log("testdo_load: Start");
        ...
        for (i = 0; i <= 6; i++) {
            if (registerValues[i] != expectedValues[i]) {
                log("REG VALS [" + i + "] HEX " + Long.toString(registerValues[i], 16) + "  DEC " + registerValues[i]);
                success = false;
            }
        }
        assert (success == true);
        log("testdo_load: Passed");
    }
}
