...
public class ARMV7AssemblerTest extends MaxTestCase {
    ...
    // public void adc(final ConditionFlag cond, final boolean s, final CiRegister Rd, final CiRegister Rn, final int
// immed_8, final int rotate_amount)
    ...
    public void testAdc() throws Exception {
        ...
        /*
         * int instructions[] = new int[3]; int value,i,j,k; setBitMasks(-1,MaxineARMTester.BitsFlag.All32Bits); for(j =
         * 0; j < 17;j++) testvalues[j] = false; int srcReg = 0; for(int destReg = 4; destReg < 6; destReg++) {
         * testvalues[destReg] = true; testvalues[destReg-1] = false; for(j = 14; j <16;j++){
         * asm.movt(ARMV7Assembler.ConditionFlag.Always,ARMV7.cpuRegisters[destReg],valueTestSet[j]);
         * asm.adc(ARMV7Assembler
         * .ConditionFlag.Always,true,ARMV7.cpuRegisters[destReg],ARMV7.cpuRegisters[srcReg],valueTestSet[j] <<7,2); //
         * The left shift of 7 means that the valueTestValues used will always be less than 8bits (255)
         *
         * } } // testing conditionflags (signed/unsigned) for( i=0; i < ARMV7Assembler.ConditionFlag.values().length;
         * i++) { asm.adc(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0],
         * armv7.arch.registers[1], 0, 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x02A10000 |
         * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
         * asm.adc(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1], 0,
         * 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x02B10000 | ARMV7Assembler.ConditionFlag.values()[i].value()
         * <<28)); asm.codeBuffer.reset(); }
         */
    }
    ...
    // The following values will be uddated
    ...
    private boolean generateAndTest(int assemblerStatements, long expected[], boolean tests[], MaxineARMTester.BitsFlag masks[]) throws Exception {
    ...
    public void testMulSdivUdiv() {
         ...
         * subroutine int assemblerStatements = 15; int i,instructions [] = new int [assemblerStatements];
         * initialiseExpectedValues(); setBitMasks(-1, MaxineARMTester.BitsFlag.All32Bits); setTestValues(-1, false);
         * System.out.println("TESTING  MUL SDIV UDIV HIGH probability of bugs!!!"); asm.codeBuffer.reset(); for(i = 1;
         * i < 8;i++) { // 2*8= 16 testvalues[i-1] = true; if(i%2 == 0) { expectedValues[i-1] = 2*i;
         * asm.mov32BitConstant(ARMV7.cpuRegisters[i-1], (int) i*2); }else { expectedValues[i-1] = -i*2;
         * asm.mov32BitConstant(ARMV7.cpuRegisters[i-1], (int) -i*2); } } // 1
         ...
         *
         *
         * expectedValues[8] = -8;testvalues[8] = true; //expectedValues[9] = 7;testvalues[9] = true; expectedValues[10]
         * = -1;testvalues[10] = true; //expectedValues[11] = 1;testvalues[11] = true;
         *
         *
        ...
        for (int srcReg = 0; srcReg < 3; srcReg++) {
            for (int destReg = 0; destReg < 3; destReg++) {
                ...
                for (int i = 0; i < scratchTestSet.length; i++) {
                    ...
                    /*
                     * //System.out.println("SRCREG " + srcReg+ " DESTREG " + destReg + " EXPECT " +
                     * expectedValues[destReg]);
                     *
                     * //ARMCodeWriter.debug = true; code = new ARMCodeWriter(3,instructions); ARMCodeWriter.debug =
                     * false; MaxineARMTester r = new MaxineARMTester(expectedValues,testvalues,bitmasks);
                     * r.assembleStartup(); r.assembleEntry(); r.compile(); r.link(); r.objcopy(); r.runSimulation();
                     */
                }
                if (destReg == 2)
                    testvalues[2] = false;
    ...
    public void testVPUSHPOP() throws Exception {
        ...
        setBitMasks(-1, MaxineARMTester.BitsFlag.All32Bits);
        setTestValues(-1, false);
        System.out.println("TESTING  vpush vpop... only tested SINGLE PRECISION !!!!. needs more extensive testing of encodings and asserts HIGH probability of bugs!!!");
        asm.codeBuffer.reset();
        for (int i = 0; i < 5; i++) { // 5*5 = 25
            asm.mov32BitConstant(ARMV7.cpuRegisters[i], (int) i);
        ...
        expectedValues[4] = 4;
        testvalues[4] = true;
        assert(generateAndTest(assemblerStatements, expectedValues, testvalues, bitmasks));
    }
    ...
    public void testVDIV() throws Exception {
        ...
        setBitMasks(-1, MaxineARMTester.BitsFlag.All32Bits);
        setTestValues(-1, false);
        System.out.println("TESTING  VDIV.... needs more extensive testing of encodings and asserts HIGH probability of bugs!!!");
        asm.codeBuffer.reset();
        asm.mov32BitConstant(ARMV7.cpuRegisters[0], (int) 10);
        asm.mov32BitConstant(ARMV7.cpuRegisters[1], (int) 24);
    ...
    @SuppressWarnings("cast")
    public void testVMOV() throws Exception{
    ...
    public void testFloatIngPointExperiments() throws Exception {
        /*
         * Will create a few simple examples (using hardcoded emitInt()) to verify concepts/principles then will add the
         * instructions
         */
        int assemblerStatements = 11;
        initialiseExpectedValues();
        ...
        testvalues[0] = true;
        assert(true == generateAndTest(assemblerStatements, expectedValues, testvalues, bitmasks));
    }
    ...
    public void testdecq() throws Exception {
        /*
         * strd and ldrd refer to the store/load register dual the versions encoded here are for a base register value
         * plus/minus an 8 bit immediate. int instructions[] = new int
         */
    ...
    public void testaddq() throws Exception {
        ...
        generateAndTest(assemblerStatements, expectedValues, testvalues, bitmasks);
        /*
         * for(int j = 0; j < assemblerStatements;j++) { instructions[j] = asm.codeBuffer.getInt(j*4); }
         * ARMCodeWriter.debug = false;
         *
         * code = new ARMCodeWriter(assemblerStatements,instructions); MaxineARMTester r = new
         * MaxineARMTester(expectedValues,testvalues,bitmasks);
         *
         * r.assembleStartup(); r.assembleEntry(); r.compile(); r.link(); r.objcopy(); r.runSimulation();
         */
    }
    ...
    public void testldrsh() throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            asm.ldrshw(ARMV7Assembler.ConditionFlag.Always, 1, 1, 0, ARMV7.cpuRegisters[i], ARMV7.cpuRegisters[13], i * 2); // stackpointer advanced by 8
            testvalues[i] = true;
            if (i < 2) {
    ...
    public void testldrb() throws Exception {
        ...
        for (int i = 0; i < 8; i++) {
            asm.ldrb(ARMV7Assembler.ConditionFlag.Always, 1, 1, 0, ARMV7.cpuRegisters[i], ARMV7.cpuRegisters[13], i); // stackpointer advanced by 8
            testvalues[i] = true;
            if (i < 4) {
    ...
    public void teststrdAndldrd() throws Exception {
        ...
        for (int i = 0; i < 10; i += 2) {
            ...
            /*
             * for(int j = 0; j < 10;j++) { instructions[j] = asm.codeBuffer.getInt(j*4); } ARMCodeWriter.debug = false;
             *
             * code = new ARMCodeWriter(10,instructions); MaxineARMTester r = new
             * MaxineARMTester(expectedValues,testvalues,bitmasks);
             *
             * r.assembleStartup(); r.assembleEntry(); r.compile(); r.link(); r.objcopy(); r.runSimulation();
             */
    ...
    public void testpushAndPop() throws Exception {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (i < 13) {
                testvalues[i] = true; // test register values r0..r12
            }
        }
        ...
        for (int bitmask = 1; bitmask <= 0xfff; bitmask = bitmask | (bitmask + 1), registers++) {
             ...
             *
             * ARMCodeWriter.debug = false; code = new ARMCodeWriter(41,instructions);
             *
             * try { MaxineARMTester r = new MaxineARMTester(expectedValues,testvalues,bitmasks);
             *
             * r.assembleStartup(); r.assembleEntry(); r.compile(); r.link(); r.objcopy(); r.runSimulation(); } catch
             * (Exception e) { System.err.println(e); e.printStackTrace(); System.exit(-1); }
             */
    ...
    /*
     * public void testsetUpScratch() throws Exception { int instructions[] = new int[1]; int value,i,j;
     * setBitMasks(-1,MaxineARMTester.BitsFlag.All32Bits); for(j = 0;j <17;j++) testvalues[j] = false; testvalues[12-1]
     * = true; i = 0;
     *
     * for( j = 0; j< scratchTestSet.length;j++) {
     *
     * //value < 65536;value++) { value = scratchTestSet[j]; expectedValues[destReg] = ((long)value);
     * asm.setUpScratch(new CiAddress(), value); instructions[0] = asm.codeBuffer.getInt(0);
     *
     * code = new ARMCodeWriter(1,instructions); MaxineARMTester r = new
     * MaxineARMTester(expectedValues,testvalues,bitmasks); r.assembleStartup(); r.assembleEntry(); r.compile();
     * r.link(); r.objcopy(); r.runSimulation();
     *
     * assertTrue(asm.codeBuffer.getInt(0) == (0x03400000 | (ARMV7Assembler.ConditionFlag.Always.value() <<28) |(destReg
     * << 12)| (value & 0xfff) | ((value & 0xf000) << 4))); asm.codeBuffer.reset(); } }
     *
     * }
     */
    ...
    public void testMovror() throws Exception {
        ...
        for (int shift = 1; shift <= 31; shift++) {
             ...
             *
             *
             * code = new ARMCodeWriter(4,instructions); MaxineARMTester r = new
             * MaxineARMTester(expectedValues,testvalues,bitmasks); r.assembleStartup(); r.assembleEntry(); r.compile();
             * r.link(); r.objcopy(); r.runSimulation();
             */
            mask = mask | (mask + 1);
    ...
    public void testmovw() throws Exception {
        ...
        /*
         * APN we are only doing condition code testing for .Always we might extend this testing later
         * ARMV7Assembler.ConditionFlag.values()[i] only testing the movw to registers 0 to 12 inclusive
         */
        for (int destReg = 0; destReg < 13; destReg++) {
            ...
            for (int j = 0; j < valueTestSet.length; j++) {
                 ...
                 *
                 * code = new ARMCodeWriter(1,instructions); MaxineARMTester r = new
                 * MaxineARMTester(expectedValues,testvalues,bitmasks); r.assembleStartup(); r.assembleEntry();
                 * r.compile(); r.link(); r.objcopy(); r.runSimulation();
                 */
    ...
    public void testmovt() throws Exception {
        ...
        /*
         * APN we are only doing condition code testing for .Always we might extend this testing later only testing the
         * movw to registers 0 to 12 inclusive
         */
        for (int destReg = 0; destReg < 13; destReg++) {
    ...
    public void testAdclsl() throws Exception {
        for (int i = 0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
            asm.adclsl(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1], armv7.arch.registers[2], 0);
            assertTrue(asm.codeBuffer.getInt(0) == (0x00A10002 | ARMV7Assembler.ConditionFlag.values()[i].value() << 28));
            asm.codeBuffer.reset();
            asm.adclsl(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1], armv7.arch.registers[2], 0);
     ...
     * armv7.arch.registers[2], 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00810062 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.addror(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00910062 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     *
     * public void testBiclsr() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.biclsr(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01C10332 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.biclsr(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01D10332 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     *
     * public void testCmnasr() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.cmnasr(ARMV7Assembler.ConditionFlag.values()[i], armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01700251 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); }
     *
     * }
     *
     * public void testCmnror() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.cmnror(ARMV7Assembler.ConditionFlag.values()[i], armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01700271 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     *
     * public void testCmpasr() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.cmpasr(ARMV7Assembler.ConditionFlag.values()[i], armv7.arch.registers[0], armv7.arch.registers[1], 0);
     * assertTrue(asm.codeBuffer.getInt(0) == ( 0x01500041 | ARMV7Assembler.ConditionFlag.values()[i].value() <<28));
     * asm.codeBuffer.reset(); } }
     *
     * public void testEorlsr() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.eorlsr(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00210332|
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.eorlsr(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00310332 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */
    /*
     * public void testMvnror() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.mvnror(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01E00271|
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.mvnror(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01F00271 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */

    /*
     * public void testOrrlsl() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.orrlsl(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01810002|
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.orrlsl(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x01910002 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */

    /*
     * public void testRsb() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.rsb(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1], 0, 0);
     * assertTrue(asm.codeBuffer.getInt(0) == ( 0x02610000 | ARMV7Assembler.ConditionFlag.values()[i].value() <<28));
     * asm.codeBuffer.reset(); asm.rsb(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0],
     * armv7.arch.registers[1], 0, 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x02710000|
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */
    /*
     * public void testRsblsl() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.rsblsl(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00610002 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.rsblsl(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], 0); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00710002 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */

    /*
     * public void testRsc() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.rsc(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00E10002 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.rsc(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00F10002 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */
    /*
     * public void testRsclsr() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.rsclsr(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00E10332 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.rsclsr(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00F10332 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */
    /*
     * public void testSbcror() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.sbcror(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00C10372 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.sbcror(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00D10372 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     */

    /*
     * public void testTst() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     ...
     * asm.codeBuffer.reset(); } }
     *
     * public void testTstlsr() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     ...
     * asm.codeBuffer.reset(); } }
     *
     * public void testSmlal() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.smlal(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00E10293|
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.smlal(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00F10293 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     *
     * public void testUmull() throws Exception { for(int i=0; i < ARMV7Assembler.ConditionFlag.values().length; i++) {
     * asm.umull(ARMV7Assembler.ConditionFlag.values()[i], false, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00810293|
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset();
     * asm.umull(ARMV7Assembler.ConditionFlag.values()[i], true, armv7.arch.registers[0], armv7.arch.registers[1],
     * armv7.arch.registers[2], armv7.arch.registers[3]); assertTrue(asm.codeBuffer.getInt(0) == ( 0x00910293 |
     * ARMV7Assembler.ConditionFlag.values()[i].value() <<28)); asm.codeBuffer.reset(); } }
     *
