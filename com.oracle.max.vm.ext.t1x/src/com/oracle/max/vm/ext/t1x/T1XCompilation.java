...
public abstract class T1XCompilation {
    ...
    public void compileT1XTesting(ClassMethodActor method, CodeAttribute codeAttribute, byte [] fakeBytes, int hardStop) {
        /*
        Begin of initCompile fake ....
        anything  commented out has been removed because it is
         not yet required or handled or breaks things in the T1X test harness --
          the frame is the ONLY THING FAKED SO FAR SEE ARMT1XTest and initialiseFrameForCompilation
         */
        assert this.method == null;
        assert buf.position() == 0;
        assert objectLiterals.isEmpty();
        ...
        // initHandlers(method, code);
        /*
        End of initCompile fake
         */
        /*
         * TODO APN fill in the apropriate calls to correctly and appropriately fake the stack frame and to correctly
         * initialise the adapter / prologues. This might not be possible until we take in a "real" java compiled class method.
         */
        emitPrologue();
