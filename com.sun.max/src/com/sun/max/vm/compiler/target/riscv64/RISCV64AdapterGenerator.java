...
/**
 * Adapters for RISCV64 baseline and optimised stack frames.
 *
 */
public abstract class RISCV64AdapterGenerator extends AdapterGenerator {
    ...
    public RISCV64AdapterGenerator(Adapter.Type adapterType) {
        super(adapterType);
        scratch = opt.getScratchRegister();
    }

    public static class Baseline2Opt extends RISCV64AdapterGenerator {
        ...
        public static class Baseline2OptAdapter extends Adapter {
            ...
            private int computeFrameState(StackFrameCursor cursor) {
                assert false : "Not currently implemented on RISCV64";
                return 0;
            }
            ...
            public void advance(StackFrameCursor current) {
                ...
                if (MaxineVM.isHosted()) {
                    assert false : "Not implemented yet in RISCV64";
        ...
        /**
         * Frame layout for an RISCV64 specific {@link Type#BASELINE2OPT} adapter frame.
         * <pre>
         *
         *          +------------------------+
         *          |  Baseline caller RIP   |
        ...
        protected int emitPrologue(Object out, Adapter adapter) {
            RISCV64MacroAssembler masm = out instanceof OutputStream ? new RISCV64MacroAssembler(Platform.target(), null) : (RISCV64MacroAssembler) out;
            if (adapter == null) {
        ...
        protected Adapter create(Sig sig) {
            ...
            RISCV64MacroAssembler masm = new RISCV64MacroAssembler(Platform.target(), null);
     ...
     *
     */
    public static class Opt2Baseline extends RISCV64AdapterGenerator {
        ...
        static final class Opt2BaselineAdapter extends Adapter {
            ...
            private int computeRipAdjustment(StackFrameCursor cursor) {
                assert false : "Not yet implemented in RISCV64";
                return 0;
            }
        ...
        /**
         * Frame layout for an RISCV64 specific {@link Type#OPT2BASELINE} adapter frame.
         * <pre>
         *
         *          +------------------------+
         *          |     OPT caller RIP     |
        ...
        /*
         * (non-Javadoc)
         * @see com.sun.max.vm.compiler.target.riscv64.RISCV64AdapterGenerator#create(com.sun.max.vm.compiler.target.AdapterGenerator.Sig)
         */
        @Override
        protected Adapter create(Sig sig) {
            CiValue[] optArgs = opt.getCallingConvention(JavaCall, WordUtil.ciKinds(sig.kinds, true), target(), false).locations;
     ...
     * @return
     */
    protected int adaptArgument(RISCV64MacroAssembler asm, Kind kind, CiValue optArg, int baselineStackOffset32, int adapterFrameSize) {
     ...
     * @param adapterFrameSize
     */
    protected abstract void adapt(RISCV64MacroAssembler asm, Kind kind, int optStackOffset32, int baselineStackOffset32, int adapterFrameSize);
    protected abstract void adapt(RISCV64MacroAssembler asm, Kind kind, CiRegister reg, int offset32);
