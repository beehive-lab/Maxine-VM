...
/**
 * The {@code ARMV&Backend} class represents the backend for the ARMV7 architecture.
 */
public class ARMV7Backend extends Backend {
    ...
    public LIRGenerator newLIRGenerator(C1XCompilation compilation) {
        return new ARMV7LIRGenerator(compilation);
    }
    ...
    public LIRAssembler newLIRAssembler(C1XCompilation compilation, TargetMethodAssembler tasm) {
        return new ARMV7LIRAssembler(compilation, tasm);
    }
    ...
    public CiXirAssembler newXirAssembler() {
        return new ARMV7XirAssembler(compiler.target);
    }
    ...
    public CompilerStub emit(Id stub) {
        ...
        try {
            return new ARMV7CompilerStubEmitter(comp, stub.arguments, stub.resultKind).emit(stub);
        } finally {
            comp.close();
        }
    }
    ...
    public CompilerStub emit(CiRuntimeCall rtCall) {
        final C1XCompilation comp = new C1XCompilation(compiler, null, -1, null, DebugInfoLevel.FULL);
        try {
            return new ARMV7CompilerStubEmitter(comp, rtCall.arguments, rtCall.resultKind).emit(rtCall);
        } finally {
            comp.close();
        }
    }
    ...
    public CompilerStub emit(XirTemplate t) {
        final C1XCompilation comp = new C1XCompilation(compiler, null, -1, null, DebugInfoLevel.FULL);
        try {
            return new ARMV7CompilerStubEmitter(comp, getArgumentKinds(t), t.resultOperand.kind).emit(t);
        } finally {
            comp.close();
        }
    }
