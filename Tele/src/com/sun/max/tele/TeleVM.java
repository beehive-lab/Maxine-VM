 ...
 * Company, Ltd.
 */
/*VCSID=ca0faa76-f4c7-494f-9ed9-6766516ba690*/
...
public abstract class TeleVM implements VMAccess {
    ...
    // Factory for creating fake object providers that represent Java objects living in the JDWP server.
    ...
    private static MaxineVM createVM(BootImage bootImage) {
        ...
        final VMConfiguration vmConfiguration = new VMConfiguration(b.buildLevel(), b.platform(),
    ...
    protected abstract Pointer loadBootHeap() throws IOException;

    public boolean isBootImageRelocated() {
        return true;
    }

    private final Pointer _bootHeapStart;

    public final Pointer bootHeapStart() {
        return _bootHeapStart;
    }

    private final Pointer _bootHeapEnd;

    private final Pointer _bootCodeStart;

    public final Pointer bootCodeStart() {
        return _bootCodeStart;
    }

    private final Pointer _bootCodeEnd;
    ...
    protected TeleVM(File bootImageFile, BootImage bootImage, Classpath sourcepath, String[] commandlineArguments, int id) throws BootImageException, IOException {
        ...
        _bootHeapStart = loadBootHeap();
        _bootHeapEnd = _bootHeapStart.plus(bootImage.header()._bootHeapSize);
        _bootCodeStart = _bootHeapEnd;
        _bootCodeEnd = _bootCodeStart.plus(bootImage.header()._bootCodeSize);
    ...
    public static TeleVM createNewChild(File bootImageFile, Classpath sourcepath, String[] commandlineArguments, int id) throws BootImageException, IOException {
        final BootImage bootImage = new BootImage(bootImageFile);
        switch (bootImage.vmConfiguration().platform().operatingSystem()) {
            case DARWIN:
                return new DarwinTeleVM(bootImageFile, bootImage, sourcepath, commandlineArguments, id);
            case LINUX:
                return new LinuxTeleVM(bootImageFile, bootImage, sourcepath, commandlineArguments, id);
            case SOLARIS:
                return new SolarisTeleVM(bootImageFile, bootImage, sourcepath, commandlineArguments, id);
            case GUESTVM:
                return new GuestVMXenTeleVM(bootImageFile, bootImage, sourcepath, commandlineArguments, id);
            default:
    ...
    public static TeleVM createWithNoProcess(File bootImageFile, Classpath sourcepath, boolean relocating) throws BootImageException, IOException {
        final BootImage bootImage = new BootImage(bootImageFile);
        return new NoTeleMaxineVM(bootImageFile, bootImage, sourcepath, relocating);
    }
    ...
    private boolean _updatingMemoryRegions;
    ...
    private boolean heapContains(Address address) {
        ...
        if (_bootHeapStart.lessEqual(address) && _bootHeapEnd.greaterThan(address)) {
            return true;
        }
        return dynamicHeapContains(address);
    }
    ...
    public boolean codeContains(Address address) {
        ...
        if (_bootCodeStart.lessEqual(address) && _bootCodeEnd.greaterThan(address)) {
            return true;
        }
        if (_codeRegions != null) {
            for (MemoryRegion codeRegion : _codeRegions) {
                if (codeRegion.contains(address)) {
                    return true;
                }
            }
        }
    ...
    public Reference getReference(Address pointer, int index) {
        return wordToReference(teleProcess().dataAccess().getWord(pointer, 0, index));
    }
    ...
    public String getString(Reference stringReference) {
        if (interpreterUseLevel() > 1) {
            final TeleReferenceValue stringReferenceValue = TeleReferenceValue.from(this, stringReference);
            final int length = InspectorInterpreter.start(this, String.class, "length", SignatureDescriptor.fromJava(int.class), stringReferenceValue).toInt();
            final char[] chars = new char[length];
            for (int i = 0; i < length; i++) {
                chars[i] = InspectorInterpreter.start(this, String.class, "charAt", SignatureDescriptor.fromJava(char.class, int.class),
    ...
    /**
     * Gets a canonical local {@link ClassActor} for the named class, creating one if needed by loading the class
     * from the classpath using the {@link PrototypeClassLoader#PROTOTYPE_CLASS_LOADER}.
     *
    ...
    private ClassActor makeClassActor(String name) throws ClassNotFoundException {
        // The VM registry includes all ClassActors for classes loaded locally using the prototype class loader
        ClassActor classActor = ClassRegistry.vmClassRegistry().get(JavaTypeDescriptor.getDescriptorForJavaString(name));
        if (classActor == null) {
            ...
            if (name.endsWith("[]")) {
                ...
                classActor = PrototypeClassLoader.PROTOTYPE_CLASS_LOADER.makeClassActor(JavaTypeDescriptor.getDescriptorForWellFormedTupleName(name));
            }
        }
        return classActor;
    }
    ...
    /**
     * Gets a canonical local {@link ClassActor} corresponding to a {@link ClassActor}
     * in the {@link TeleVM}, creating one if needed by loading the class using the
     * {@link PrototypeClassLoader#PROTOTYPE_CLASS_LOADER} from
     * either the classpath, or if not found on the classpath, by copying the
     * classfile from the {@link TeleVM}.
     *
     * @param classActorReference A {@link ClassActor} in the {@link TeleVM}.
     * @return Local, equivalent {@link ClassActor}, possibly created by loading
     * from the classpath, or if not found, by copying and loading the classfile from the {@link TeleVM}
     */
    public ClassActor makeClassActor(Reference classActorReference) {
        ...
        try {
            ...
            // Not loaded and not available on local classpath; load by copying classfile from the {@link TeleVM}.
            ...
            TeleError.check(teleByteArrayObject != null, "could not find class actor: " + name);
    ...
    /**
     * Gets a canonical local {@classActor} corresponding to the type of a heap object
     * in the targetVM, creating one if needed by loading the class using the
     * {@link PrototypeClassLoader#PROTOTYPE_CLASS_LOADER} from
     * either the classpath, or if not found on the classpath, by copying the
     * classfile from the {@link TeleVM}.
     *
    ...
    public Hub makeLocalHubForObject(Reference objectReference) {
        ...
        return (StaticHub.class.isAssignableFrom(hubClassActor.toJava())) ? objectClassActor.staticHub() : objectClassActor.dynamicHub();
    }
    ...
    /**
     * Updates the set of types that are available on the {@linkplain #classpath() class path} by scanning the class
     * path. This scan will be performed automatically the first time {@link #loadableTypeDescriptors()} is called.
     * However, it should also be performed any time the set of classes available on the class path may have
     * changed.
     */
    public void updateLoadableTypeDescriptorsFromClasspath() {
        ...
        Trace.end(1, "searching classpath for class files [" + typesOnClasspath.size() + " types found]");
        _typesOnClasspath = typesOnClasspath;
    }

    public Reference bootClassRegistryReference() {
        return originToReference(_bootHeapStart.plus(_bootImage.header()._classRegistryOffset));
    }
    ...
    public void advanceToJavaEntryPoint() throws IOException {
        _messenger.enable();
        final Address startEntryPoint = bootHeapStart().plus(bootImage().header()._vmRunMethodOffset);
        try {
    ...
    /**
     * @return a collection of all current threads in the targetVM, ordered by threadID.
     */
    ...
    /**
     * Refreshes the values that describe what code and heap regions are currently defined in the VM.
     */
    private void refreshMemoryRegions() {
        _updatingMemoryRegions = true;
        _heapRegions = null;
        _codeRegions = null;
        try {
            final Reference heapRegionsArrayReference = readReference(_bootHeapStart.plus(bootImage().header()._heapRegionsPointerOffset));
            final TeleArrayObject teleHeapRegionsArray = (TeleArrayObject) TeleObject.make(this, heapRegionsArrayReference);
            if (teleHeapRegionsArray != null) {
                _heapRegions = (MemoryRegion[]) teleHeapRegionsArray.deepCopy();
            }
            final Reference codeRegionsArrayReference = readReference(_bootHeapStart.plus(bootImage().header()._codeRegionsPointerOffset));
            final TeleArrayObject teleCodeRegionsArray = (TeleArrayObject) TeleObject.make(this, codeRegionsArrayReference);
            if (teleCodeRegionsArray != null) {
                _codeRegions = (MemoryRegion[]) teleCodeRegionsArray.deepCopy();
            }
        } finally {
            _updatingMemoryRegions = false;
        }
    }
    ...
    /**
     * Identifies the most recent GC for which the local copy of the inspection table in the {@link TeleVM} is valid.
     */
    ...
    @Override
    public void dispose() {
        // TODO: Consider implementing disposal of the VM when told so by a JDWP command.
    ...
    /**
     * Tries to find a JDWP ObjectProvider that represents the object that is referenced by the parameter.
     * @param reference a reference to the object that should be represented as a JDWP ObjectProvider
     * @return a JDWP ObjectProvider object or null, if no object is found at the address specified by the reference
     */
    ...
    public ReferenceTypeProvider[] getReferenceTypesBySignature(String signature) {
        ...
        // Always fake the Object type. This means that calls to all methods of the Object class will be reflectively delegated to the Object class that lives
        ...
        // Try to find a matching class actor that lives within the VM based on the signature.
        final AppendableSequence<ReferenceTypeProvider> result = new LinkSequence<ReferenceTypeProvider>();
        for (TypeDescriptor td : teleClassRegistry().typeDescriptors()) {
        ...
        // If no class living in the VM was found, try to lookup Java class known to the JDWP server. If such a class is found, then a JDWP
        // reference type is faked for it.
        if (result.length() == 0) {
            try {
                final Class klass = JavaTypeDescriptor.resolveToJavaClass(JavaTypeDescriptor.parseTypeDescriptor(signature), this.getClass().getClassLoader());
                result.append(_javaProviderFactory.getReferenceTypeProvider(klass));
            } catch (NoClassDefFoundError e) {
                LOGGER.log(Level.SEVERE, "Error while looking up class based on signature", e);
            }
        }
    ...
    public void resume() {
        ...
        if (this.teleProcess().state() == TeleProcess.State.STOPPED) {
            ...
            if (_registeredSingleStepThread != null) {
                ...
                // There has been a thread registered for performing a single step => perform single step instead of resume.
                try {
                    ...
                    LOGGER.log(Level.SEVERE, "Unexpected error while performing a single step in the VM", e);
            ...
            } else if (_registeredStepOutThread != null && _registeredStepOutThread.getReturnAddress() != null) {

                // There has been a thread registered for performing a step out => perform a step out instead of resume.
                ...
                try {
                    ...
                    LOGGER.log(Level.SEVERE, "Unexpected error while performing a run-to-instruction in the VM", e);
                } catch (InvalidProcessRequestException e) {
                    LOGGER.log(Level.SEVERE, "Unexpected error while performing a run-to-instruction in the VM", e);
                }
                ...
                try {
                    ...
                    LOGGER.log(Level.SEVERE, "Unexpected error while resuming the VM", e);
                } catch (InvalidProcessRequestException e) {
                    LOGGER.log(Level.SEVERE, "Unexpected error while resuming the VM", e);
                }
            }
    ...
    public void suspend() {
        ...
        if (this.teleProcess().state() == TeleProcess.State.RUNNING) {
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, "Unexpected error while pausing the VM", e);
            } catch (InvalidProcessRequestException e) {
                LOGGER.log(Level.SEVERE, "Unexpected error while pausing the VM", e);
            }
        } else {
    ...
    private StateTransitionListener _model = new StateTransitionListener() {
        ...
        public void handleStateTransition(StateTransitionEvent event) {
            if (event.newState() == State.TERMINATED) {
                fireVMDiedEvent();
            } else if (event.newState() == State.STOPPED && !_listeners.isEmpty()) {
     ...
     */
    private void fireSingleStepEvent(ThreadProvider thread, CodeLocation location) {
        LOGGER.info("VM EVENT: Single step was made at thread " + thread + " to location " + location);
        for (VMListener l : _listeners) {
     ...
     */
    private void fireBreakpointEvent(ThreadProvider thread, CodeLocation location) {
        LOGGER.info("VM EVENT: Breakpoint hit at thread " + thread + " at location " + location);
        for (VMListener l : _listeners) {
     ...
     * Always sets the breakpoint at the call entry point of a method. Does ignore the suspendAll parameter, there will always be all threads
     * suspended when the breakpoint is hit.
    ...
    public void addBreakpoint(CodeLocation codeLocation, boolean suspendAll) {
        ...
        this.teleProcess().targetBreakpointFactory().makeBreakpoint(tma.getCurrentJavaTargetMethod().callEntryPoint(), false);
        Trace.line(1, "Breakpoint set at: " + tma.getCurrentJavaTargetMethod().callEntryPoint());
    }

    @Override
    public void removeBreakpoint(CodeLocation l) {
        final TeleClassMethodActor tma = (TeleClassMethodActor) l.method();
        this.teleProcess().targetBreakpointFactory().removeBreakpointAt(tma.getCurrentJavaTargetMethod().callEntryPoint());
    ...
    private final ThreadGroupProvider _javaThreads = new ThreadGroupProviderImpl(true);
    private final ThreadGroupProvider _nativeThreads = new ThreadGroupProviderImpl(false);

    /**
     * Class representing a thread group used for logical grouping in the JDWP protocol. Currently we only distinguish between
    ...
    public Value readValue(Kind kind, Pointer pointer, int offset) {
        ...
        final Reference reference = teleProcess().teleVM().originToReference(pointer);

        if (kind == Kind.REFERENCE) {
            final Word word = teleProcess().dataAccess().readWord(pointer, offset);
            return TeleReferenceValue.from(teleProcess().teleVM(), teleProcess().teleVM().wordToReference(word));
        }
        ...
        if (result.kind() == Kind.REFERENCE && !this.isValidOrigin(result.asReference().toOrigin())) {
            LOGGER.severe("Wrong reference encountered (" + result.asReference() + "), returning null reference!");
    ...
    /**
     * Converts a value as seen by the Maxine VM to a value as seen by the JDWP server.
    ...
    public VMValue convertToVirtualMachineValue(Value value) {
        switch (value.kind().asEnum()) {
            ...
            case REFERENCE:
                return this.createObjectProviderValue(this.findObject(value.asReference()));
            case SHORT:
            ...
            case WORD:
                ...
                LOGGER.warning("Tried to convert a word, this is not implemented yet! (word=" + word + ")");
                return this.getVoidValue();
        }
    ...
    public Value convertToValue(VMValue vmValue) {
        if (vmValue.isVoid()) {
            ...
            throw new IllegalArgumentException("Could not convert the provider object " + p + " to a reference!");
    ...
    public void registerSingleStepThread(TeleNativeThread teleNativeThread) {
        ...
        if (_registeredSingleStepThread != null) {
            LOGGER.warning("Overwriting registered single step thread! " + _registeredSingleStepThread);
        }
    ...
    public void registerStepOutThread(TeleNativeThread teleNativeThread) {
        if (_registeredStepOutThread != null) {
            LOGGER.warning("Overwriting registered step out thread! " + _registeredStepOutThread);
        }
     ...
     *
     * @param klass the class object whose JDWP reference type should be looked up
    ...
    /**
     * Converts a value kind as seen by the Maxine world to a VMValue type as seen by the VM interface used by the JDWP server.
     *
    ...
    public static Type toVirtualMachineType(Kind kind) {
        ...
        throw new IllegalArgumentException("Typeype " + kind + " cannot be resolved to a virtual machine value type");
    }
    ...
    public TeleTargetRoutine findTeleTargetRoutine(Address address) {
        ...
        TeleTargetRoutine result = teleCodeRegistry().get(TeleTargetRoutine.class, address);
        if (result == null) {
            LOGGER.info("No target method found for address " + address + ", trying to create new one");
