...
public abstract class ReferenceMapInterpreter {
    ...
    void pushRef(int extraSlots) {
        assert sp < (maxStack() + extraSlots);
        updateStack(sp++, true);
    }
        ...
        while (bci != endBCI) {
            ...
            switch (opcode) {
                ...
                case INVOKESTATIC: {
                    ...
                    /*
                     * Method handle static adapter methods can be identified by
                     * the presence of an appendix argument. In such cases the appendix
                     * will have been stacked, so the adjustment is made to the stack
                     * reference map here.
                     */
                    Object appendix = constantPool.lookupAppendix(index, opcode);

                    if (appendix != null) {
                        // MethodHandle.LinkTo* methods increase the stack by one slot.
                        pushRef(1);
                    }
                    ...
                    if (opcode != Bytecodes.INVOKESTATIC) {
                        /* invokeBasic invocations are generated by
                         * java.lang.invoke.InvokerBytecodeGenerator.emitInvoke(Name name) which is supposed to produce
                         * the bytecode that will eventually run invokeVirtual on invokeBasic. This code although
                         * claiming (in comments) to be pushing the receiver it does not appear to manipulate the stack.
                         * As a result, in such cases we should not pop the receiver.
                         */
                        if (appendix == null && !methodConstant.name(constantPool).string.startsWith("invokeBasic")) {
