...
import com.sun.max.vm.code.NativeOrVmIP;
...
import com.sun.max.vm.log.hosted.VMLoggerInterface;
...
import static com.sun.max.vm.compiler.target.Stub.Type.CompilerStub;
 ...
 * <li>Any vtable or itable entry denoting a method in <i>M</i> is reverted to a {@link Stubs#virtualTrampoline(int)
 * vtable} or {@link Stubs#interfaceTrampoline(int) itable} trampoline.</li>
 * <li>Patch the entry points of all invalidated methods to {@linkplain TargetMethod#redirectTo(TargetMethod) redirect}
 * the caller to the {@linkplain Stubs#staticTrampoline() static trampoline}. This ensures all direct calls to an
 * invalidated method will be re-linked the next time they are executed.</li>
 ...
 * <p/>
 * This occurs when one of the deoptimization stubs is executed. A deoptimization stub is executed either as the result
 * of the return address patching described above or by a (compiled) call to the {@linkplain Stubs#genUncommonTrapStub()
 * uncommon trap} stub. All the deoptimization stubs eventually route through to
 * {@link #deoptimize(CodePointer, Pointer, Pointer, Pointer, CiCalleeSaveLayout, CiConstant) deoptimize()} which
 * constructs the deoptimized frames, unrolls them onto the stack and continues execution in the appropriate deoptimized
 * frame. </i>
 * </ul>
 */
public class Deoptimization extends VmOperation {
    ...
    static {
        ...
        VMOptions.addFieldOption("-XX:", "DeoptimizeALot", Deoptimization.class,
                        "Invalidate and deoptimize a selection of executing optimized methods every <n> milliseconds. " + "A value of 0 disables this mechanism.");
    }
    ...
    protected void doIt() {
        ...
        while (i < methods.size()) {
            ...
            if (!tm.invalidate(new InvalidationMarker(tm))) {
                ...
                // Find all references to invalidated target method(s) in dispatch tables (e.g. vtables, itables etc)
                // and revert to trampoline references.
    ...
    public static class Info extends com.sun.max.vm.stack.RawStackFrameVisitor {
        ...
        /**
         * The address in the stack at which the {@linkplain #slots slots} of the deoptimized frame(s) are unrolled.
         */
        Pointer slotsAddr;
        ...
        /**
         * The actual return address in frame being deoptimized. This will differ from {@link #callerIP} when the caller
         * is (also) marked for deoptimization.
         */
    ...
    /**
     * Deoptimizes a method that is being returned to via a {@linkplain Stubs#deoptStub(CiKind, boolean) deoptimization
     * stub}.
     *
    ...
    public static void deoptimize(CodePointer ip, Pointer sp, Pointer fp, Pointer csa, CiCalleeSaveLayout csl, CiConstant returnValue) {
        ...
        if (pendingException != null) {
            ...
            assert topFrame != null : "could not (re)find handler for " + pendingException + " thrown at " + tm + "+" + ip.to0xHexString();
        }
...
// START GENERATED CODE
    private static abstract class DeoptLoggerAuto extends com.sun.max.vm.log.VMLogger {
        ...
        public enum Operation {
            ALot, CatchException, Continuation, DoIt, Frames, PatchITable, PatchReturnAddress, PatchVTable, Start, TmPos, Unroll;
        ...
        @INLINE
        public final void logPatchReturnAddress(TargetMethod targetMethod, Object callee, Stub stub, CodePointer to, Pointer save, Pointer patch, CodePointer from) {
            log(Operation.PatchReturnAddress.ordinal(), objectArg(targetMethod), objectArg(callee), objectArg(stub), codePointerArg(to), save, patch, codePointerArg(from));
        }

        protected abstract void tracePatchReturnAddress(TargetMethod targetMethod, Object callee, Stub stub, CodePointer to, Pointer save, Pointer patch, CodePointer from);
