...
public class FixedAddressCodeManager extends CodeManager {
    ...
    @Override
    void initialize() {
        final Address baselineAddress = Code.bootCodeRegion().end().roundedUpBy(Platform.platform().pageSize);
        tryAllocate(runtimeBaselineCodeRegionSize, runtimeBaselineCodeRegion, baselineAddress);
        final Address optAddress = runtimeBaselineCodeRegion.end().roundedUpBy(Platform.platform().pageSize);
    ...
    private void tryAllocate(VMSizeOption s, CodeRegion cr, Address address) {
        ...
        if (!VirtualMemory.allocateAtFixedAddress(address, size, VirtualMemory.Type.CODE)) {
            ProgramError.unexpected("could not allocate " + cr.regionName());
        }
