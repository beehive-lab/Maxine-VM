 ...
 * Much leaner than {@link NativeConditionVariable} and {@link NativeMutex}.
 */
public class OSMonitor {
    ...
    public static Word newMutex() {
        Word mutex = Memory.mustAllocate(Size.fromInt(getMutexSize()));
        nativeMutexInitialize(mutex);
        return mutex;
    }

    public static Word newCondition() {
        Word condition = Memory.allocate(Size.fromInt(getConditionSize()));
