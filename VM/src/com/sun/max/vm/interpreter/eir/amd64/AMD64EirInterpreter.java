 ...
 * Company, Ltd.
 */
/*VCSID=e9438022-c7c9-4887-92e7-729fd2883c9d*/
...
public class AMD64EirInterpreter extends EirInterpreter implements AMD64EirInstructionVisitor {
    ...
    private AMD64EirCPU _cpu = new AMD64EirCPU(this);
    ...
    protected Value interpret(EirMethod eirMethod, Value... arguments) throws InvocationTargetException {
        final AMD64EirCPU savedCPU = _cpu.save();
        try {
    ...
    public void visit(CMP_I64 instruction) {
        Value valueA = _cpu.read(instruction.destinationOperand().location());
        Value valueB = _cpu.read(instruction.sourceOperand().location());
        final long a;
        final long b;
    ...
    public void visit(CQO instruction) {
        ...
        final long rax = _cpu.readLong(instruction.sourceOperand().location());
        final long rdx = (rax < 0L) ? -1L : 0L;
        _cpu.writeLong(instruction.destinationOperand().location(), rdx);
    }
    ...
    public void visit(CVTSI2SD_I64 instruction) {
        final long n = _cpu.readInt(instruction.sourceOperand().location());
        _cpu.writeDouble(instruction.destinationOperand().location(), n);
    }
    ...
    public void visit(CVTTSS2SI_I64 instruction) {
        final float f = _cpu.readFloat(instruction.sourceOperand().location());
        _cpu.writeLong(instruction.destinationOperand().location(), (long) f);
    }

    public void visit(DEC_I64 instruction) {
        long n = _cpu.readLong(instruction.operand().location());
        n--;
        _cpu.writeLong(instruction.operand().location(), n);
    }

    private static final BigInteger MAX_ULONG64 = new BigInteger("FFFFFFFFFFFFFFFF", 16);

    public void visit(DIV_I64 instruction) {
        final long rd = _cpu.readLong(RDX);
        final long dividend = _cpu.readLong(RAX);
        ...
        if (dividend < 0) {
            ...
            _cpu.writeLong(RAX, quotientAndRemainder[0].longValue());
            _cpu.writeLong(RDX, quotientAndRemainder[1].longValue());
        } else {
            _cpu.writeLong(RAX, dividend / divisor);
            _cpu.writeLong(RDX, dividend % divisor);
        }
    }
    ...
    public void visit(IDIV_I32 instruction) {
        final int divisor = _cpu.readInt(instruction.divisor().location());
        long dividend = _cpu.readInt(RDX);
        dividend <<= 32;
        dividend |= _cpu.readInt(RAX);
        _cpu.writeInt(RAX, (int) (dividend / divisor));
        _cpu.writeInt(RDX, (int) (dividend % divisor));
    }
    ...
    public void visit(JMP_indirect instruction) {
        final int serial = _cpu.readWord(instruction.operand().location()).asAddress().toInt();
        for (EirBlock block : instruction.block().method().blocks()) {
            if (block.serial() == serial) {
                _cpu.gotoBlock(block);
                return;
            }
        }
    ...
    public void visit(MOVD_I64_F64 instruction) {
        final double d = _cpu.readDouble(instruction.sourceOperand().location());
        _cpu.writeLong(instruction.destinationOperand().location(), UnsafeLoophole.doubleToLong(d));
    }

    public void visit(MOVD_F32_I32 instruction) {
        final int f = _cpu.readInt(instruction.sourceOperand().location());
        _cpu.writeFloat(instruction.destinationOperand().location(), UnsafeLoophole.intToFloat(f));
    }

    public void visit(MOVD_F64_I64 instruction) {
        final long d = _cpu.readLong(instruction.sourceOperand().location());
        _cpu.writeDouble(instruction.destinationOperand().location(), UnsafeLoophole.longToDouble(d));
    }
    ...
    public void visit(MOVSXD instruction) {
        final int i = _cpu.readInt(instruction.sourceOperand().location());
        _cpu.writeLong(instruction.destinationOperand().location(), i);
    }
    ...
    public void visit(MOVZXD instruction) {
        final int i = _cpu.readInt(instruction.sourceOperand().location());
        _cpu.writeLong(instruction.destinationOperand().location(), i & 0xffffffffL);
    }
    ...
    public void visit(POP instruction) {
        final Value result = _cpu.pop();
        _cpu.write(instruction.destinationOperand().location(), result);
    }

    public void visit(PUSH instruction) {
        final Value value = _cpu.read(instruction.sourceLocation());
        _cpu.push(value);
    }
    ...
    public void visit(SAR_I64 instruction) {
        assert !(instruction.sourceOperand().location() instanceof AMD64EirRegister.General) || (instruction.sourceOperand().location() == RCX);
        final long number = _cpu.readLong(instruction.destinationOperand().location());
        final int shift = _cpu.readByte(instruction.sourceOperand().location());
        _cpu.writeLong(instruction.destinationOperand().location(), number >> shift);
    }

    private void setConditionally(AMD64EirUnaryOperation instruction, boolean condition) {
        _cpu.writeLong(instruction.operand().location(), condition ? 1L : 0L);
    }

    public void visit(SETB instruction) {
        setConditionally(instruction, _cpu.test(CF));
    }

    public void visit(SETBE instruction) {
        setConditionally(instruction, _cpu.test(CF) || _cpu.test(ZF));
    }

    public void visit(SETL instruction) {
        setConditionally(instruction, _cpu.test(SF) != _cpu.test(OF));
    }
    ...
    public void visit(SETNP instruction) {
        setConditionally(instruction, !_cpu.test(PF));
    }
    ...
    public void visit(XCHG instruction) {
        final Word a = _cpu.readWord(instruction.destinationOperand().location());
        final Word b = _cpu.readWord(instruction.sourceOperand().location());
        _cpu.writeWord(instruction.destinationOperand().location(), b);
        _cpu.writeWord(instruction.sourceOperand().location(), a);
    }
    ...
    public void visit(ZERO instruction) {
        switch (instruction.kind().asEnum()) {
            ...
            case WORD:
                ...
                break;
            case REFERENCE:
                _cpu.write(instruction.operand().location(), ReferenceValue.NULL);
                break;
            case FLOAT:
