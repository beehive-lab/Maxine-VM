 ...
 * Company, Ltd.
 */
/*VCSID=f8a31172-1174-49d9-a964-d055cc47d776*/
...
public final class TargetBreakpoint {
    ...
    public boolean isEnabled() {
        return _originalCode != null;
    }

    private void enable() {
        if (_originalCode == null) {
            final byte[] breakpointCode = breakpointCode();
            _originalCode = new byte[breakpointCode.length];
            ...
            Memory.readBytes(_instructionPointer, _originalCode);
            Memory.writeBytes(breakpointCode, _instructionPointer);
        }
    }

    private void disable() {
        if (_originalCode != null) {
            Memory.writeBytes(_originalCode, _instructionPointer);
            _originalCode = null;
        }
    }
     ...
     */
    private static final SortedLongArrayMapping<TargetBreakpoint> _targetBreakpoints = new SortedLongArrayMapping<TargetBreakpoint>();
    ...
    public static byte[] findOriginalCode(long instructionPointer) {
        final TargetBreakpoint targetBreakpoint = _targetBreakpoints.get(instructionPointer);
        if (targetBreakpoint != null) {
            return targetBreakpoint._originalCode;
        }
        return null;
    }

    public static synchronized void make(Address instructionPointer) {
        TargetBreakpoint targetBreakpoint = _targetBreakpoints.get(instructionPointer.toLong());
        if (targetBreakpoint == null) {
            targetBreakpoint = new TargetBreakpoint(instructionPointer);
            _targetBreakpoints.put(instructionPointer.toLong(), targetBreakpoint);
        }
        targetBreakpoint.enable();
    }

    public static synchronized void delete(Address instructionPointer) {
        final TargetBreakpoint targetBreakpoint = _targetBreakpoints.get(instructionPointer.toLong());
        if (targetBreakpoint != null) {
            targetBreakpoint.disable();
            _targetBreakpoints.remove(instructionPointer.toLong());
        }
    }
}
