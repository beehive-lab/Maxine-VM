...
/**
 * Specialized folding semantics for Java builtins.
 *
 * @author Doug Simon
 */
public class CirJavaBuiltin extends CirBuiltin {

    @HOSTED_ONLY
    public CirJavaBuiltin(JavaBuiltin builtin) {
        super(builtin);
    }
    ...
    public boolean isFoldable(CirOptimizer cirOptimizer, CirValue[] arguments) {
        for (int i = 0; i < arguments.length - 2; i++) {
            if (!arguments[i].isConstant() || arguments[i].kind() == Kind.REFERENCE) {
                return false;
            }
        }
        return true;
