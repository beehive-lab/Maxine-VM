 ...
 * Company, Ltd.
 */
/*VCSID=f5d58026-3996-4f1c-9511-46c8449ff00b*/
package com.sun.max.vm.compiler.dir.eir.ia32;
...
import com.sun.max.vm.compiler.eir.allocate.ia32.*;
 ...
 */
public final class DirToIA32EirMethodTranslation extends DirToEirMethodTranslation {

    public DirToIA32EirMethodTranslation(EirGenerator eirGenerator, EirMethod eirMethod, DirMethod dirMethod) {
        super(eirGenerator, eirMethod, dirMethod);
    }

    @Override
    protected EirAllocator createAllocator(EirMethodGeneration methodGeneration) {
        return new IA32EirSomeAllocator(methodGeneration);
    }

    @Override
    protected DirToEirInstructionTranslation createInstructionTranslation(EirBlock eirBlock) {
        return new DirToIA32EirInstructionTranslation(this, eirBlock);
    }

    @Override
    protected EirPrologue createPrologue(EirBlock eirBlock) {
        return new IA32EirPrologue(eirBlock, eirMethod(),
    ...
    protected EirEpilogue createEpilogue(EirBlock eirBlock) {
        return new IA32EirEpilogue(eirBlock, eirMethod(),
                                    calleeSavedEirVariables(), calleeSavedEirRegisters(),
                                    resultEirLocation());
    }
    ...
    public EirInstruction createAssignment(EirBlock eirBlock, Kind kind, EirValue destination, EirValue source) {
        final EirInstruction<?, ?> assignment = new IA32EirAssignment(eirBlock, kind, destination, source);
        return assignment;
    }

    @Override
    public EirSafepoint createSafepoint(EirBlock eirBlock) {
        return new IA32EirSafepoint(eirBlock);
    }
}
