...
//    }
     ...
     * {@inheritDoc}
     * <p>
     * Default deep copying behavior is to use local copies of {@link ClassMethodActor} in
     * place of those in the VM, since they are presumed to be equivalent by virtue of being
     * loaded from the same class files.  In particular, debugging information generated by
     * the compiler is embedded in the {@link ClassMethodActor}'s {@link CodeAttribute}.
     * <p>
     * This creates an inconsistency in situations where the standard bytecodes (especially
     * for auto-generated native stubs) in the VM are modified and recompiled, for example
     * for tracing.
     * <p>
     * In those special situations we dynamically <em>patch</em> the local
     * {@link ClassMethodActor} instance with an instance of {@link CodeAttribute}
     * copied from the VM.  The best time to do that is when we know we are going to
     * need the debugging information, but as late as possible so that anything done to the
     * code in the VM is already complete.  This would be when the deep copy of the whole
     * target method gets created.d
     */
    @Override
    protected Object createDeepCopy(DeepCopier context) {
        final Object copy = super.createDeepCopy(context);
        if (context.level() == 1) {
            // Finished copying.  Check for special cases.
            if (teleClassMethodActor != null &&
                            teleClassMethodActor.isNative() &&
                            !teleClassMethodActor.isSubstituted()) {
                // Native and non-substituted.  If this code is auto-generated, but hasn't
                // yet been generated in the VM, then the auto-generation machinery will
                // eventually generate it in the local copy.  That code might not be
                // the same as what's used in the VM, for example if auto-generated
                // stubs are modified for tracing.
                // Use the actual code from the VM, which we know already exists because
                // we're already dealing with a TargetMethod for it.
                teleClassMethodActor.useCodeAttributeFromVM();
            }
        }
        return copy;
    }
