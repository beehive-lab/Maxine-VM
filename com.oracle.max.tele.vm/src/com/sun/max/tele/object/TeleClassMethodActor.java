...
public abstract class TeleClassMethodActor extends TeleMethodActor implements MethodProvider {
   ...
   /**
    ...
    /**
     * Patch the local copy of this {@link ClassMethodActor} with a copy of the
     * {@link ClassMethodActor#codeAttribute()} from the VM.  These are usually
     * the same, but there are occasional situations where the VM's copy is being
     * managed specially, for example when the auto-generated bytecodes for a stub
     * are dynamically modified with tracing code before being compiled.
     * <p>
     * This will only be useful at a point when that code is in place.
     */
    public void useCodeAttributeFromVM() {
        final ClassMethodActor classMethodActor = classMethodActor();
        if (classMethodActor == null) {
            ...
            final CodeAttribute codeAttribute = (CodeAttribute) teleCodeAttribute.deepCopy();
