...
public class BytecodeToAMD64TargetTranslator extends BytecodeToTargetTranslator {
    ...
    private static boolean TrimNops2 = true;
    ...
    static {
        ...
        VMOptions.addFieldOption("-XX:", "TrimNops2", BytecodeToAMD64TargetTranslator.class, "Trim as many nops as possible");
    ...
    @INLINE
    private void addExtraNops(int numBytesNeeded) {
    ...
    private void alignPatchableCallSite(int callSitePosition) {
        ...
        addExtraNops(WordWidth.BITS_64.numberOfBytes - (callSitePosition & alignment));
    }

    @Override
    protected void alignDirectBytecodeCall(TargetMethod template, ClassMethodActor callee) {
        final int alignment = WordWidth.BITS_64.numberOfBytes - 1;
        int numBytesNeeded = codeBuffer.currentPosition() & alignment;
        if (numBytesNeeded == 0) {
        ...
        alignPatchableCallSite(codeBuffer.currentPosition() + template.stopPosition(0));
    }
    ...
    protected void alignTemplateWithPatchableSite(TargetMethod template) {
        ...
        // Compute number of bytes needed to aligned the whole template
        numBytesNeeded =  WordWidth.BITS_64.numberOfBytes - numBytesNeeded;
        ...
        // Alignment is needed only for unresolved direct call (i.e., those that call directly to trampoline),
        // since resolved direct calls are never patched again.
        // So here we loop over the direct calls and skips resolved one and aligned one until the first unresolved
        // unaligned is found. If none, we can return, otherwise, we align.

        if (TrimNops2) {
            Object [] directCallees = template.directCallees();
            int firstUnresolved = 0;
            for (; firstUnresolved < numDirecCalls; firstUnresolved++) {
                if (template.getTargetMethod(directCallees[firstUnresolved]) == null) {
                    if (firstUnresolved == (numDirecCalls - 1)) {
                        alignPatchableCallSite(codeBuffer.currentPosition() + template.stopPosition(firstUnresolved));
                        return;
                    }
                    // Otherwise, there are more than one call site. We need to make sure that the whole template is aligned to 8-byte
                    // boundary as this guarantees that all call site within the template are within a cache line.
                    addExtraNops(numBytesNeeded);
                    return;
                }
            }
            // All direct calls are resolved. No need to align.
            alignmentAvoidedCount++;
            return;
        } else if (TrimNops && numDirecCalls == 1) {
