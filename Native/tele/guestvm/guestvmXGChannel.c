...
JNIEXPORT jboolean JNICALL
Java_com_sun_max_tele_debug_guestvm_GuestVMXGNativeTeleChannelProtocol_nativeGatherThreads(JNIEnv *env, jclass c, jobject teleDomain, jobject threadSeq, jlong tlaList, jlong primordialTLA) {
    tele_xg_gather_threads();
    ...
    while (tcb != NULL) {
            ...
            threadLocals = teleProcess_findTLA(&xg_memory_handler, tlaList, primordialTLA, tcb->regs.u.xregs_64.rsp, threadLocals, &nativeThreadLocalsStruct);
