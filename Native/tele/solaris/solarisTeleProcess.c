...
static int gatherThread(void *data, const lwpstatus_t *ls) {
    ...
    TLA tla = teleProcess_findTLA(a->ph, a->tlaList, a->primordialTLA, stackPointer, threadLocals, &nativeThreadLocalsStruct);
...
JNIEXPORT void JNICALL
Java_com_sun_max_tele_channel_natives_TeleChannelNatives_gatherThreads(JNIEnv *env, jobject  this, jlong processHandle, jobject teleProcess, jobject threadList, long tlaList, long primordialTLA) {
    struct ps_prochandle *ph = (struct ps_prochandle *) processHandle;
